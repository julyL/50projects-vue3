...
const itemList = reactive([
  ...
  { name: 'Locations' },
  {
    ...
    children: [{ name: 'Netflix culture memo' }, { name: 'Work life balance' }, { name: 'Inclusion & diversity' }, { name: 'Blog' }],
  },
]);

function handlerClick(bool) {
  if (bool) active.value = !active.value;
}
</script>
    ...
    <div class="nav nav-black" :class="[visible ? 'visible' : '']">
      <div class="nav nav-red" :class="[visible ? 'visible' : '']">
        <div class="nav nav-white" :class="[visible ? 'visible' : '']">
          ...
          <ul class="list">
            <li v-for="(item, index) in itemList" :key="index" @click="handlerClick(item.children && item.children.length)">
              <a href="#">{{ item.name }}</a>
              <i v-if="item.children" class="icon fas" :class="[active ? 'fas fa-angle-down' : 'fa-angle-right']" />
