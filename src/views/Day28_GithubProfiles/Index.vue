...
async function getUser(username: string): Promise<void> {
  try {
    ...
    loading.value = false;
    if (+error?.response?.status === 404) errMsg.value = 'No profile with this username';
  }
}
...
function handleSubmit() {
  loading.value = true;
  if (username.value) getUser(username.value);
}
</script>
...
<template>
  <div class="body">
    <form id="form" class="user-form" @submit.prevent="handleSubmit">
      <input v-model.trim.lazy="username" type="text" placeholder="Search a Github User" />
    </form>

    <main id="main">
      <div v-if="!Object.getOwnPropertyNames(user).length && !errMsg && loading">Loading...</div>

      <div v-else-if="Object.getOwnPropertyNames(user).length && !errMsg" class="card">
        <div>
        ...
        <div class="user-info">
          ...
          <div id="repos">
            <a v-for="repo in repoList" :key="repo.id" :href="repo.html_url" class="repo" target="_blank">{{ repo.name }}</a>
          </div>
        </div>
      </div>
