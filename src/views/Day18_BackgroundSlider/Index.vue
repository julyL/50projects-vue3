...
function leftClick(): void {
  ...
  if (activeSlide.value < 0) activeSlide.value = imgList.length - 1;
...
function rightClick(): void {
  ...
  if (activeSlide.value > imgList.length - 1) activeSlide.value = 0;
...
<template>
  <div class="body" :style="{ backgroundImage: `url(${bgImgUrl})` }">
    <div class="slider-container">
      <div v-for="(img, index) in imgList" :key="index" class="slide" :class="[img.active ? 'active' : '']" :style="{ backgroundImage: `url(${img.url})` }" />
