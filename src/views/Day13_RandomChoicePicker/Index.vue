...
<template>
  <div class="body">
    <div class="container">
      ...
      <textarea id="textarea" v-model.trim="text" v-focus placeholder="Enter choices here..." @keyup="handleKeyup" />

      <div id="tags">
        <span v-for="(item, index) in tag.list" :key="index" class="tag" :class="[item.highlight ? 'highlight' : '']">{{ item.value }}</span>
      </div>
    </div>
  </div>
</template>
