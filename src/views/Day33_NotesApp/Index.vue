...
onMounted(() => {
  notesList.value = JSON.parse(localStorage.getItem('noteslist')) ? JSON.parse(localStorage.getItem('noteslist')) : [];
});
...
<template>
  <div class="body">
    <button id="add" class="add" @click="addNote"><i class="fas fa-plus" /> Add note</button>
    ...
    <div v-for="(note, index) in notesList" :key="index" class="note">
      ...
      <textarea v-model.lazy="note.text" :class="note.text && note.showTextArea ? 'hidden' : ''" />
    </div>
  </div>
</template>
