...
function highlightCups(idx: number) {
  ...
  else if (cupsList[idx].isFull && !cupsList[idx + 1].isFull) idx--;

  cupsList.forEach((cup, idx2) => {
    if (idx2 <= idx) cup.isFull = true;
    else cup.isFull = false;
  });
}
  ...
  <div class="body">
    <div class="left_area">
      ...
      <div class="cup">
        ...
        <div v-show="fullCups" class="percentage" :style="{ height: `${percentageHeight}px` }">{{ proportion }}%</div>
      </div>
    </div>
    <div class="right_area">
      ...
      <div class="cups">
        <div v-for="(cup, idx) in cupsList" :key="idx" class="cup cup-small" :class="[!cup.isFull || 'full']" @click="highlightCups(idx)">
          {{ cup.capacity }}
          ml
        </div>
      </div>
