...
class ScorePanel {
  ...
  addScore() {
    ...
    if (this.score.value % this.levelStep === 0) this.levelUp();
  }

  levelUp() {
    if (this.level.value < this.maxLevel) this.levelEl.textContent = `${++this.level.value}`;
  }
}
...
class Snake {
  ...
  checkBodyHeader() {
    for (let i = 1; i < this.bodies.length; i++) {
      ...
      if (this.X === bd.offsetLeft && this.Y === bd.offsetTop) throw new Error('game over');
    }
  }
...
class GameControl {
  ...
  run() {
    ...
    let X = this.snake.X;
    let Y = this.snake.Y;
    if (this.direction === 'ArrowUp') Y -= 10;

    if (this.direction === 'ArrowDown') Y += 10;

    if (this.direction === 'ArrowLeft') X -= 10;

    if (this.direction === 'ArrowRight') X += 10;
    ...
    // 注意等级
    this.isLive && (this.timer = setTimeout(this.run.bind(this), 300 - (this.scorePanel.level.value - 1) * 30));
  }
